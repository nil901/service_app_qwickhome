...
class PunchOutScreen extends StatefulWidget {
  ...
  const PunchOutScreen({super.key, required this.bookingId});
...
class _PunchOutScreenState extends State<PunchOutScreen> {
  ...
  Future pick() async {
    ...
    setState(() => completeImage = File(picked.path));
  }

  Future punchOut() async {
    if (completeImage == null) {
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Add completion photo")));
      return;
    }
    ...
    FormData body = FormData.fromMap({
      ...
      "completionImage": await MultipartFile.fromFile(
        completeImage!.path,
        filename: p.basename(completeImage!.path),
      )
    });
    ...
    if (res.data["success"] == true) {
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Punched Out Successfully ✓")));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text("Punch Out")),
      body: Padding(
        padding: EdgeInsets.all(16),
        child: Column(
          children: [
            ElevatedButton(onPressed: pick, child: Text("Add Final Photo")),

            if (completeImage != null)
              Image.file(completeImage!, height: 120),
